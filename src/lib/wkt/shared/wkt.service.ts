...
export class WktService {
  ...
  public snrcWKT(snrc, epsgTO = 'EPSG:3857') {
    ...
    if (/\d{2,3}[a-l][0,1][0-9]/gi.test(snrc)) {
      ...
      coord['lr'] = [coord['ul'][0] + unitPerType_EW, coord['ul'][1] - unitPerType_SN];
      ...
      wktPoly = 'POLYGON((' + [coord.ul.join(' '),
      coord['ur'].join(' '), coord['lr'].join(' '),
      coord['ll'].join(' '), coord.ul.join(' ')].join(',') + '))';
